...
#define RL_LOC   147
  ...
  if (tempHour > 9)
    drawMyString(workTime, -50, TIME_LOC, offset);  //10 - 12
  else
    drawMyString(workTime, 50, TIME_LOC, offset);  // 0 -  9

  // Animate rainbow shimmer on the digits.
...
{ // Copied from Test_8
  ...
  for (int i = 0; i < digitCount[digit]; i++) {
    ...
    //    Serial.printf("digit=%d i=%d seg=%d\r\n", digit, i, seg);
    //    Serial.flush();
...
{ // From Test_8, renamed to work here.
  ...
  //  Serial.print("drawMyString called with: /"); Serial.print(str);
  //  Serial.println("/");
  // Convert 24 hour time to 12 hour time.
  tempHour = tm->tm_hour;
  //  Serial.printf("tempHour %i (before) - ", tempHour);
  if (tempHour == 0) tempHour = 12;  // Midnight
  if (tempHour > 12) tempHour -= 12;  // 1:00 pm through 11:59 pm
  ...
  for (int position = 0; str[position] != '\0'; position++) {
    ...
    if (str[position] == ':') {
      ...
      tft.drawString(":", digitX + 3, digitY);
      if (tempHour > 9)
        digitX += 20;
      else
        digitX += 30;
...
//    if (iLooper++ > 30) {
//      Serial.println("Cannot get time after waiting a minute. Rebooting.");
//      ESP.restart();
//    }
//  }
...
/*****************************************************************************/
{
  ...
  switch (which)
  {
    ...
    case 1:
      {
        Serial.println("Collapsing and reviving rectangles");
    ...
    case 2:
      {
        Serial.println("Concentric Circles (Ripple Effect)");
        int w = tft.width();
        int h = tft.height();
        int cx = w / 2;
        int cy = h / 2;
    ...
    case 4:
      {
        Serial.println("Expanding Filled Rectangles (Pulse)");
        int w = tft.width();
        int h = tft.height();
        static int rainbowOffset = 0;
    ...
    case 6:
      {
        Serial.println("Vertical Bars Pulse");
        int w = tft.width();
        int h = tft.height();
        static int rainbowOffset = 0;
    ...
    case 7:
      {
        Serial.println("Starburst Lines");
        int w = tft.width();
        int h = tft.height();
        int cx = w / 2;
        int cy = h / 2;
    ...
    case 8:
      {
        Serial.println("Checkerboard Flash");
        int w = tft.width();
        int h = tft.height();
        int size = 20;
        static int rainbowOffset = 0;
    ...
    case 10:
      {
        Serial.println("Fill with Circles");
        int w = tft.width();
        int h = tft.height();
        int cx = w / 2;
        int cy = h / 2;
    ...
    case 11:
      {
        Serial.println("Fill with Boxes");
        int w = tft.width();
        int h = tft.height();
